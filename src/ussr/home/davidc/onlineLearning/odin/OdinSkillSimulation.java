...
public abstract class OdinSkillSimulation extends GenericSimulation implements PhysicsObserver {
    ...
    private static ArrayList<WorldDescription.ModulePosition> ballPos = new ArrayList<WorldDescription.ModulePosition>();
    ...
    private static void addModule(int i, int j, String type) {
    	if(isNeighorBalls(ballPos.get(i),ballPos.get(j))) {
			...
			modulePos.add(new WorldDescription.ModulePosition(Integer.toString(constructIndex),type, pos, rot));
	...
	private static void addBall(int x, int y, int z) {
    	...
    	ballPos.add(new WorldDescription.ModulePosition(Integer.toString(constructIndex),"OdinBall", pos, new RotationDescription(0,0,0)));
