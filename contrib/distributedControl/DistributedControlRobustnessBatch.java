...
public class DistributedControlRobustnessBatch extends SimulationBatch2000 {
    ...
    private List<Class<? extends DCRobustnessExperiment>> experiments = new ArrayList<Class<? extends DCRobustnessExperiment>>();
    private PrintWriter logfile;
    
    public static void main(String argv[]) {
        new DistributedControlRobustnessBatch(DCSettings.EXPERIMENTS).start(DCSettings.N_PARALLEL_SIMS,true);
    }
    ...
    public DistributedControlRobustnessBatch(Class<?>[] mainClasses) {
        int counter = 0;
        for(int ci=0; ci<mainClasses.length; ci++) {
            // Efficiency experiments, 0% failure risk, varying packet loss
            if(!DCSettings.SKIP_EFFICIENCY)
                for(float risk = DCSettings.START_RISK; risk<=DCSettings.END_RISK; risk+=DCSettings.RISK_INC) {
                    for(int i=0; i<DCSettings.N_REPEAT; i++) {
                        parameters.add(new DistributedControlRobustnessBatch.Parameters(mainClasses[ci],counter,Math.max(0, risk-DCSettings.RISK_DELTA),risk,0,0,DCSettings.TIMEOUT,0,0));
                    }
                    counter++;
                }
            if(!DCSettings.SKIP_EFFICIENCY_EXTRA) {
                newRandomSequence();
                for(float risk = DCSettings.START_RISK; risk<=DCSettings.END_RISK; risk+=DCSettings.RISK_INC) {
                    resetRandomSequence();
                    for(int i=0; i<DCSettings.N_REPEAT; i++) {
                        parameters.add(new DistributedControlRobustnessBatch.Parameters(mainClasses[ci],counter,Math.max(0, risk-DCSettings.RISK_DELTA),risk,0,0,DCSettings.TIMEOUT,0,0,nextRandomFromSequence()));
                    }
                    counter++;
                }
            }
            // Robustness experiments, varying failure risk, no packet loss
            if(!DCSettings.SKIP_ROBUSTNESS) {
                newRandomSequence();
                for(float fail = DCSettings.START_FAIL; fail<=DCSettings.END_FAIL; fail+=DCSettings.FAIL_INC) {
                    resetRandomSequence();
                    for(int i=0; i<DCSettings.N_REPEAT; i++) {
                        parameters.add(new DistributedControlRobustnessBatch.Parameters(mainClasses[ci],counter,DCSettings.FAIL_COMM_RISK,DCSettings.FAIL_COMM_RISK,fail,DCSettings.COMPLETE_FAILURE_DEGREE,DCSettings.TIMEOUT,0,0,nextRandomFromSequence()));
                    }
                    counter++;
                }
            }
            if(!DCSettings.SKIP_RESET) {
                newRandomSequence();
                for(float interval = DCSettings.RESET_RISK_TS_SIZE_MIN; interval<=DCSettings.RESET_RISK_TS_SIZE_MAX; interval+=DCSettings.RESET_RISK_TS_SIZE_DELTA)
                    for(float reset = DCSettings.RESET_RISK_PER_TS_MIN; reset<=DCSettings.RESET_RISK_PER_TS_MAX; reset+=DCSettings.RESET_RISK_PER_TS_DELTA) {
                        resetRandomSequence();
                        for(int i=0; i<DCSettings.N_REPEAT; i++)
                            parameters.add(new Parameters(mainClasses[ci],counter,0,0,0,0,DCSettings.TIMEOUT,reset,interval,nextRandomFromSequence()));
                        counter++;
                    }
            }
        }
