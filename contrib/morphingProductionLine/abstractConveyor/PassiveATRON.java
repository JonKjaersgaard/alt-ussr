...
class PassiveATRON extends ATRON {
     ...
     * 
     */
    protected final AbstractMPLSimulation simulation;
    protected boolean isActive = false;
    /**
     * @param simulation
     */
    PassiveATRON(AbstractMPLSimulation simulation) {
    ...
    protected class PassiveController extends ATRONController {
        ...
        protected void symmetricDisconnect(int connector) {
            this.sendMessage(AbstractMPLSimulation.MSG_DISCONNECT_HERE, AbstractMPLSimulation.MSG_DISCONNECT_HERE_SIZE, (byte)connector);
        ...
        public void handleMessage(byte[] message, int messageSize, int channel) {
            if(Arrays.equals(message,AbstractMPLSimulation.MSG_DISCONNECT_HERE))
                this.disconnect(channel);
            else if(Arrays.equals(message, AbstractMPLSimulation.MSG_LIFT_ME)) {
                //System.out.println("Blocking behavior received");
                lift = channel;
                synchronized(this) { this.notify(); }
            } else if(Arrays.equals(message, AbstractMPLSimulation.MSG_IS_ACTIVE_QUERY)) { 
                if(isActive)
                    this.sendMessage(AbstractMPLSimulation.MSG_CONFIRM_ACTIVE, (byte)1, (byte)channel);
                else
                    this.sendMessage(AbstractMPLSimulation.MSG_CONFIRM_PASSIVE, (byte)1, (byte)channel);
            } else
