...
#define MAKE_USSR_CALL_INT_CONTROLLER_METHOD(RETURNTYPE,METHODNAME) RETURNTYPE METHODNAME(USSREnv *env, char *name, char *signature, ...) {\
  READ_USSR_ENV(jniEnv,self);\
  va_list parameters;\
...
MAKE_USSR_CALL_INT_CONTROLLER_METHOD(int,ussr_call_int_controller_method)
MAKE_USSR_CALL_INT_CONTROLLER_METHOD(char,ussr_call_byte_controller_method)
MAKE_USSR_CALL_INT_CONTROLLER_METHOD(int64_t,ussr_call_long_controller_method)

#define MAKE_USSR_CALL_FLOAT_CONTROLLER_METHOD(METHODNAME) float METHODNAME(USSREnv *env, char *name, char *signature, ...) {\
  READ_USSR_ENV(jniEnv,self);\
  va_list parameters;\
  jmethodID get_internal_controller_mid;\
  jobject controller;\
  ...
  va_start(parameters, signature);\
  float result = (*jniEnv)->CallFloatMethodV(jniEnv, controller, mid, parameters);\
  if((*jniEnv)->ExceptionCheck(jniEnv)) {\
    (*jniEnv)->ExceptionDescribe(jniEnv);\
    (*jniEnv)->ExceptionClear(jniEnv);\
    reportError(jniEnv,"Exception occurred during method evaluation", name, signature);\
